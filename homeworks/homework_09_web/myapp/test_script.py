...
Urls = collections.namedtuple('Urls',
                              ['method', 'url', 'headers', 'json', 'data'])

with open('request_dumps.txt', 'w') as f:
    for index, url in enumerate([
        Urls('get', 'http://localhost:8000/api/flights/list/', None, None,
             None),
        ...
        resp = requests.request(method=url.method, url=url.url, json=url.json,
                                data=url.data)
